...
exports.createPendingRegistration = async (req, res) => {
  try {
    ...
    // Reject if a pending registration already exists for this phone
    const pendingDuplicate = await Registration.findOne({ phone: phone.trim(), status: 'pending' });
    if (pendingDuplicate)
...
exports.approveRegistration = async (req, res) => {
  try {
    ...
    // Guard against duplicate students
    const existing = await Student.findOne({ phone: registration.phone });
    if (existing) {
